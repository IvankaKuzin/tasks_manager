    ...
    def clean_first_name(self):
        ...
        if not value.isalpha():
            raise ValidationError(
                "First name can't contain numbers or another symbols."
            )
...
class WorkerSearchUsernameForm(Form):
    ...
    search_field = forms.ChoiceField(
        choices=SEARCH_FIELDS, required=False, label="Search by"
    )
    search_query = forms.CharField(max_length=255, required=False, label="Search query")
...
class TaskTypeSearchForm(Form):
    name = forms.CharField(max_length=255, required=False, label="Search by name")
    ordering = forms.ChoiceField(
        choices=[
            ("name", "Name (A-Z)"),
            ("-name", "Name (Z-A)"),
...
class TagCreateForm(ModelForm):
    ...
    def clean_name(self):
        ...
        if not re.match(r"^[a-zA-Z-]+$", name):
            raise ValidationError(
                "Position name can contain only letters and ellipsis."
            )
